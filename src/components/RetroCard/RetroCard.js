...
const RetroCard = ({backGround, category, identify}) => {
    ...
    return (
      <>
      <Draggable key={`${category}-${identify}`} draggableId={state[category].columnId + String(identify)} index={identify}>
        {(provided)=> (
          <div className={`${styles["RetroCard"]} ${styles[backGround]}`}
          ref={provided.innerRef}
          {...provided.draggableProps}
          {...provided.dragHandleProps}
          >
          <textarea value={state[category].data[identify].textValue} className={styles["textbox"]} onChange={(e)=>{updateItem(e, category, identify)}} />
